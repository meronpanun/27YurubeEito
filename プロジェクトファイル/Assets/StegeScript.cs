...
public class StageScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //}
        var diff = generatedStageList[preInstantiate - 1].transform.position.x - character.position.x;
        if (diff <= 0 && Mathf.Abs(diff) > StageTipSize)
        {
            UpdateStage(currentTipIndex + preInstantiate);
        }
    }
